...
def EulerZYXDegree2R(zyx_degree):
    """
    :param zyx_degree: rotation order: z, y, x
    :return:
    """
    ...
    zyx = EulerZYXRad2R(zyx_rad)

    t_rzyx = transforms3d.euler.euler2mat(zyx_rad[0], zyx_rad[1], zyx_rad[2], 'rzyx')

    assert np.allclose(zyx, t_rzyx)
...
if __name__ == "__main__":
    ...
    eul1 = [0, 1.5, 0]
    eul2 = [0.08726646, 0.17453293, 0.26179939]
    eul3 = [-0.08726646, 0.17453293, 0.26179939]
    euls = [eul1, eul2, eul3]

    for eul in euls:
        eul = np.array(eul)
        print ("Euler:{}\n".format(eul))
        R = EulerZYXRad2R(eul.reshape(3, 1))
        print ("R in zyx order:{}\n".format(R))
        xyz_back = GetEulerRadZYX(R)
        print ("Euler back:{}\n\n".format(xyz_back))